"""
Feature Encoder for Phase Risk Predictor.
...
from .schemas import PhaseRiskContext, PhaseRiskExecutionPlan, PhaseRiskSystemState
from .config import (
    PHASE_TYPES,
    MODEL_TIERS,
    EFFORT_LEVELS,
...
class PhaseRiskFeatureEncoder:
    """
    Deterministic feature encoder for phase risk prediction.
    
    Encodes PhaseRiskContext, PhaseRiskExecutionPlan, and PhaseRiskSystemState
    ...
    - Numeric: normalized scalar features (10 features)
    ...
    def encode(
        ...
        # Retry count so far (normalized)
        features.append(
            ctx.retry_count_so_far / 
            NORMALIZATION_CONSTANTS["retry_count_max"]
        )
        ...
        features.append(
            ctx.mission_time_remaining_seconds / 
            NORMALIZATION_CONSTANTS["time_remaining_max_seconds"]
        )
